...
parser.add_argument('--dataset', type=str, default='mnist', help="mnist | svhn | cifar10 | cifar100 | tiny_imagenet | tiny1_p[1-10] | UCM_p[1-10] | AID_p[1-10] | NWPU_p[1-10] | ImageNet_p[1-10]| OOD_ImageNet_[SUN|iNaturalist|places365|dtd")
...
parser.add_argument('--ori_dataset', type=str, default='UCM_p3')  # X X X X Negative Cats
...
def config_options(options):
    ...
    return options
###############################
# if networkerror, you can use offline
os.environ['WANDB_MODE'] = 'offline'
def main_worker(options):
    options = config_options(options)
    run = wandb.init(project="prompt_clip_openset_rebuttal", dir='.', reinit=True)
    ...
    if 'mnist' in options['dataset']:
        ...
        Data = AID_OSR(datasplit = options['dataset'], dataroot=options['dataroot'], batch_size=options['batch_size'], img_size=options['img_size'], few_shot=options['few_shot'], cfg = options)
        trainloader, testloader, outloader = Data.train_loader, Data.test_loader, Data.out_loader
    elif 'UCM' in options['dataset']:
        Data = UCM_OSR(datasplit = options['dataset'], dataroot=options['dataroot'], batch_size=options['batch_size'], img_size=options['img_size'], few_shot=options['few_shot'], cfg = options)
        trainloader, testloader, outloader = Data.train_loader, Data.test_loader, Data.out_loader
    elif 'NWPU' in options['dataset']:
        Data = NWPU_OSR(datasplit = options['dataset'], dataroot=options['dataroot'], batch_size=options['batch_size'], img_size=options['img_size'], few_shot=options['few_shot'], cfg = options)
        trainloader, testloader, outloader = Data.train_loader, Data.test_loader, Data.out_loader
    elif 'tiny1' in options['dataset']:
        Data = tiny1_OSR(datasplit = options['dataset'], dataroot=options['dataroot'], batch_size=options['batch_size'], img_size=options['img_size'], few_shot=options['few_shot'], cfg = options)
        trainloader, testloader, outloader = Data.train_loader, Data.test_loader, Data.out_loader
    else:
    ...
    print('################')
    local_model_path = './model_cache/open_clip_pytorch_model.bin'
    local_model_path1 = 'model_cache/RN50.pt'
    print('################')
    # clip_model, _, _ = open_clip.create_model_and_transforms('ViT-B-16', pretrained=local_model_path, force_patch_dropout=0.5)
    clip_model, _, _ = open_clip.create_model_and_transforms('ViT-B-16', pretrained=local_model_path)
    ...
    if options['stage'] == 2 or options['stage'] == 3:
        model_positive_path = '{}/{}'.format(model_path, 'md.pth.pth')
        ...
        if 'prompt_learner.ctx_positive' in save_model.keys():
            ...
            model.get_perturbation(save_model['perturbation'])
            # model.get_tokens(save_model['trainable_tokens'])
        else:
            model.get_ctx_posi(save_model['module.prompt_learner.ctx_positive'])
            model.get_perturbation(save_model['module.perturbation'])
            # model.get_tokens(save_model['trainable_tokens'])
    ...
    if options['stage'] == 4:
        ...
        print('Stage 1 model loaded!')
        del save_model
        model_negative_path = '{}/{}'.format(model_path, 'md.pth.pth')
    ...
    file_name = 'md.pth'
    file_name1 = 'md_stage1last.pth'
    file_name2 = 'md_stage3bestauroc.pth'
    ...
    print("Acc (%): {:.3f}\t AUROC (%): {:.3f}\t OSCR (%): {:.3f}\t FPR95 (%): {:.3f}\t AUPR (%): {:.3f}\t OA (%): {:.3f}\t".format(
        results['ACC'], results['AUROC'], results['OSCR'], results['FPR95'], results['AUPR'], results['OA']
        ))
    if options['stage'] == 4 or options['stage'] == 5 or options['stage'] == 6:
    ...
    for epoch in range(options['max_epoch']):
        ...
        print(f'Duration: {epoch_time:.2f} seconds')
        # this_loss = train_clip(model, optimizer, scheduler, trainloader, run, epoch=epoch, proto = proto, **options)
        ...
        if options['eval_freq'] > 0 and (epoch+1) % options['eval_freq'] == 0 or (epoch+1) == options['max_epoch']:
            ...
            results = test_nega_clip(model, criterion, testloader, outloader, epoch=0, **options)
            # results = test_nega_clip(model, criterion, testloader, outloader, epoch=0, use_patch_dropout_in_eval=False, **options)
            print("Acc (%): {:.3f}\t AUROC (%): {:.3f}\t OSCR (%): {:.3f}\t FPR95 (%): {:.3f}\t AUPR (%): {:.3f}\t OA (%): {:.3f}\t".format(
                 results['ACC'], results['AUROC'], results['OSCR'], results['FPR95'], results['AUPR'], results['OA']))
            ...
            if results['ACC'] >= best_acc and options['LOG'] and options['stage'] == 1:
                if results['ACC'] > best_acc: 
                    ...
                    best_acc_auroc = results['AUROC'] 
                    
                    with open('save_auroc.txt', 'w') as f:
                        f.write(str(best_acc_auroc))
                    print('save:', model_path)
                    save_networks(model, model_path, file_name)
                
                elif results['ACC'] == best_acc:
                    with open('save_auroc.txt', 'r') as f:
                        saved_auroc = float(f.read() or 0)

                    if results['AUROC'] > saved_auroc:
                        best_acc_auroc = results['AUROC'] 
                        # 写入新的 AUROC 值
                        with open('save_acc.txt', 'w') as f:
                            f.write(str(best_acc_auroc))
                        print('save:', model_path)
                        save_networks(model, model_path, file_name)
                    

            if results['AUROC'] >= best_auroc and options['LOG'] and options['stage'] == 2:
                best_auroc = results['AUROC']
                print('save:', model_path)
                save_networks(model, model_path, file_name)
 
            if epoch==99 and options['LOG'] and options['stage'] == 3:
                best_auroc = results['AUROC']
                print("save:", model_path)
                save_networks(model, model_path, file_name)
